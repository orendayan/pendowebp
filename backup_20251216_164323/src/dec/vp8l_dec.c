...
static PENDO_WEBP_INLINE int GetCopyDistance(int distance_symbol,
...
static PENDO_WEBP_INLINE int GetCopyLength(int length_symbol,
                                     VP8LBitReader* const br) {
  // Length and distance prefixes are encoded the same way.
  return GetCopyDistance(length_symbol, br);
}

static PENDO_WEBP_INLINE int PlaneCodeToDistance(int xsize, int plane_code) {
...
static PENDO_WEBP_INLINE int ReadSymbol(const HuffmanCode* table,
                                  VP8LBitReader* const br) {
  int nbits;
...
static PENDO_WEBP_INLINE int ReadPackedSymbols(const HTreeGroup* group,
                                         VP8LBitReader* const br,
                                         uint32_t* const dst) {
...
static PENDO_WEBP_INLINE int GetMetaIndex(
    const uint32_t* const image, int xsize, int bits, int x, int y) {
  if (bits == 0) return 0;
  return image[xsize * (y >> bits) + (x >> bits)];
}

static PENDO_WEBP_INLINE HTreeGroup* GetHtreeGroupForPos(VP8LMetadata* const hdr,
                                                   int x, int y) {
...
static PENDO_WEBP_INLINE uint32_t Rotate8b(uint32_t V) {
#if defined(WORDS_BIGENDIAN)
...
static PENDO_WEBP_INLINE void CopySmallPattern8b(const uint8_t* src, uint8_t* dst,
...
static PENDO_WEBP_INLINE void CopyBlock8b(uint8_t* const dst, int dist, int length) {
...
static PENDO_WEBP_INLINE void CopySmallPattern32b(const uint32_t* src,
                                            uint32_t* dst,
...
static PENDO_WEBP_INLINE void CopyBlock32b(uint32_t* const dst,
                                     int dist, int length) {
