...
static PENDO_WEBP_INLINE uint64_t VP8BitWriterPos(const VP8BitWriter* const bw) {
  const uint64_t nb_bits = 8 + bw->nb_bits_;   // bw->nb_bits_ is <= 0, note
  return (bw->pos_ + bw->run_) * 8 + nb_bits;
}

// Returns a pointer to the internal buffer.
static PENDO_WEBP_INLINE uint8_t* VP8BitWriterBuf(const VP8BitWriter* const bw) {
  return bw->buf_;
}
// Returns the size of the internal buffer.
static PENDO_WEBP_INLINE size_t VP8BitWriterSize(const VP8BitWriter* const bw) {
  return bw->pos_;
}
...
static PENDO_WEBP_INLINE size_t VP8LBitWriterNumBytes(const VP8LBitWriter* const bw) {
...
static PENDO_WEBP_INLINE void VP8LPutBits(VP8LBitWriter* const bw,
