...
static PENDO_WEBP_INLINE int GetMin(int a, int b) { return (a > b) ? b : a; }
...
static PENDO_WEBP_INLINE void UpdateHisto(int histo_argb[4][256], uint32_t argb) {
...
static PENDO_WEBP_INLINE void PredictBatch(int mode, int x_start, int y,
...
static PENDO_WEBP_INLINE int GetMax(int a, int b) { return (a < b) ? b : a; }
...
static PENDO_WEBP_INLINE uint8_t NearLosslessDiff(uint8_t a, uint8_t b) {
...
// predictions.
static PENDO_WEBP_INLINE void GetResidual(
...
static PENDO_WEBP_INLINE void MultipliersClear(VP8LMultipliers* const m) {
  m->green_to_red_ = 0;
  m->green_to_blue_ = 0;
  m->red_to_blue_ = 0;
}
...
static PENDO_WEBP_INLINE uint32_t MultipliersToColorCode(
