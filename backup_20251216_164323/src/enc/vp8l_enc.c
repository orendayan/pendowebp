...
static PENDO_WEBP_INLINE uint32_t PaletteComponentDistance(uint32_t v) {
...
static PENDO_WEBP_INLINE uint32_t PaletteColorDistance(uint32_t col1, uint32_t col2) {
...
static PENDO_WEBP_INLINE void SwapColor(uint32_t* const col1, uint32_t* const col2) {
  const uint32_t tmp = *col1;
  *col1 = *col2;
  *col2 = tmp;
}

static PENDO_WEBP_INLINE int SearchColorNoIdx(const uint32_t sorted[], uint32_t color,
                                        int num_colors) {
...
static PENDO_WEBP_INLINE uint32_t HashPix(uint32_t pix) {
...
static PENDO_WEBP_INLINE void WriteHuffmanCode(VP8LBitWriter* const bw,
...
static PENDO_WEBP_INLINE void WriteHuffmanCodeWithExtraBits(
    VP8LBitWriter* const bw,
    const HuffmanTreeCode* const code,
    int code_index,
    int bits,
...
static PENDO_WEBP_INLINE uint32_t SearchColorGreedy(const uint32_t palette[],
                                              int palette_size,
                                              uint32_t color) {
  (void)palette_size;
...
static PENDO_WEBP_INLINE uint32_t ApplyPaletteHash0(uint32_t color) {
...
static PENDO_WEBP_INLINE uint32_t ApplyPaletteHash1(uint32_t color) {
  // Forget about alpha.
  return ((uint32_t)((color & 0x00ffffffu) * 4222244071ull)) >>
         (32 - PALETTE_INV_SIZE_BITS);
}

static PENDO_WEBP_INLINE uint32_t ApplyPaletteHash2(uint32_t color) {
  // Forget about alpha.
