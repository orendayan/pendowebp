...
static PENDO_WEBP_INLINE size_t SizeWithPadding(size_t chunk_size) {
  assert(chunk_size <= MAX_CHUNK_PAYLOAD);
  return CHUNK_HEADER_SIZE + ((chunk_size + 1) & ~1U);
}

// Size of a chunk including header and padding.
static PENDO_WEBP_INLINE size_t ChunkDiskSize(const WebPChunk* chunk) {
...
static PENDO_WEBP_INLINE int IsWPI(WebPChunkId id) {
  switch (id) {
