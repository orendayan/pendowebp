...
static PENDO_WEBP_INLINE uint8_t clip_8b(int v) {
...
#if !WEBP_NEON_OMIT_C_CODE
static PENDO_WEBP_INLINE void TrueMotion(uint8_t* dst, int size) {
...
static PENDO_WEBP_INLINE void Put16(int v, uint8_t* dst) {
  int j;
  for (j = 0; j < 16; ++j) {
...
static PENDO_WEBP_INLINE void Put8x8uv(uint8_t value, uint8_t* dst) {
  int j;
  for (j = 0; j < 8; ++j) {
...
static PENDO_WEBP_INLINE void DoFilter2_C(uint8_t* p, int step) {
...
static PENDO_WEBP_INLINE void DoFilter4_C(uint8_t* p, int step) {
  const int p1 = p[-2*step], p0 = p[-step], q0 = p[0], q1 = p[step];
  const int a = 3 * (q0 - p0);
...
static PENDO_WEBP_INLINE void DoFilter6_C(uint8_t* p, int step) {
...
static PENDO_WEBP_INLINE int Hev(const uint8_t* p, int step, int thresh) {
  const int p1 = p[-2*step], p0 = p[-step], q0 = p[0], q1 = p[step];
...
#if !WEBP_NEON_OMIT_C_CODE
static PENDO_WEBP_INLINE int NeedsFilter_C(const uint8_t* p, int step, int t) {
  const int p1 = p[-2 * step], p0 = p[-step], q0 = p[0], q1 = p[step];
...
#if !WEBP_NEON_OMIT_C_CODE || WEBP_NEON_WORK_AROUND_GCC
static PENDO_WEBP_INLINE int NeedsFilter2_C(const uint8_t* p,
...
#if !WEBP_NEON_OMIT_C_CODE || WEBP_NEON_WORK_AROUND_GCC
static PENDO_WEBP_INLINE void FilterLoop26_C(uint8_t* p,
...
static PENDO_WEBP_INLINE void FilterLoop24_C(uint8_t* p,
                                       int hstride, int vstride, int size,
                                       int thresh, int ithresh,
                                       int hev_thresh) {
  const int thresh2 = 2 * thresh + 1;
