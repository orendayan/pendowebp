...
static PENDO_WEBP_INLINE uint32_t GetScale(uint32_t a, int inverse) {
...
static PENDO_WEBP_INLINE uint8_t dither_hi(uint8_t x) {
  return (x & 0xf0) | (x >> 4);
}

static PENDO_WEBP_INLINE uint8_t dither_lo(uint8_t x) {
  return (x & 0x0f) | (x << 4);
}

static PENDO_WEBP_INLINE uint8_t multiply(uint8_t x, uint32_t m) {
  return (x * m) >> 16;
}

static PENDO_WEBP_INLINE void ApplyAlphaMultiply4444_C(uint8_t* rgba4444,
                                                 int w, int h, int stride,
...
static PENDO_WEBP_INLINE uint32_t MakeARGB32(int a, int r, int g, int b) {
