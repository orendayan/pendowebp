...
static PENDO_WEBP_INLINE int clip_max(int v, int max) {
  return (v > max) ? max : v;
}
...
static PENDO_WEBP_INLINE void Fill(uint8_t* dst, int value, int size) {
  int j;
  for (j = 0; j < size; ++j) {
...
static PENDO_WEBP_INLINE void VerticalPred(uint8_t* dst,
...
static PENDO_WEBP_INLINE void HorizontalPred(uint8_t* dst,
...
static PENDO_WEBP_INLINE void TrueMotion(uint8_t* dst, const uint8_t* left,
                                   const uint8_t* top, int size) {
  int y;
  if (left != NULL) {
    if (top != NULL) {
...
static PENDO_WEBP_INLINE void DCMode(uint8_t* dst, const uint8_t* left,
                               const uint8_t* top,
...
#if !WEBP_NEON_OMIT_C_CODE
static PENDO_WEBP_INLINE int GetSSE(const uint8_t* a, const uint8_t* b,
                              int w, int h) {
  int count = 0;
  int y, x;
  for (y = 0; y < h; ++y) {
...
static PENDO_WEBP_INLINE void Copy(const uint8_t* src, uint8_t* dst, int w, int h) {
  int y;
  for (y = 0; y < h; ++y) {
    memcpy(dst, src, w);
    src += BPS;
