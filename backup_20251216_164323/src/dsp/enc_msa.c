...
static PENDO_WEBP_INLINE void ITransformOne(const uint8_t* ref, const int16_t* in,
                                      uint8_t* dst) {
  v8i16 input0, input1;
...
static PENDO_WEBP_INLINE void VE4(uint8_t* dst, const uint8_t* top) {    // vertical
  const v16u8 A1 = { 0 };
...
static PENDO_WEBP_INLINE void HE4(uint8_t* dst, const uint8_t* top) {    // horizontal
  const int X = top[-1];
  const int I = top[-2];
  const int J = top[-3];
  const int K = top[-4];
...
static PENDO_WEBP_INLINE void DC4(uint8_t* dst, const uint8_t* top) {
  uint32_t dc = 4;
  int i;
...
static PENDO_WEBP_INLINE void RD4(uint8_t* dst, const uint8_t* top) {
  const v16u8 A2 = { 0 };
...
static PENDO_WEBP_INLINE void LD4(uint8_t* dst, const uint8_t* top) {
  const v16u8 A1 = { 0 };
...
static PENDO_WEBP_INLINE void VR4(uint8_t* dst, const uint8_t* top) {
  const int X = top[-1];
  const int I = top[-2];
  const int J = top[-3];
  const int K = top[-4];
...
static PENDO_WEBP_INLINE void VL4(uint8_t* dst, const uint8_t* top) {
  const int A = top[0];
  const int B = top[1];
  const int C = top[2];
  const int D = top[3];
...
static PENDO_WEBP_INLINE void HU4(uint8_t* dst, const uint8_t* top) {
  const int I = top[-2];
  const int J = top[-3];
  const int K = top[-4];
  const int L = top[-5];
...
static PENDO_WEBP_INLINE void HD4(uint8_t* dst, const uint8_t* top) {
  const int X = top[-1];
  const int I = top[-2];
  const int J = top[-3];
  const int K = top[-4];
...
static PENDO_WEBP_INLINE void TM4(uint8_t* dst, const uint8_t* top) {
  const v16i8 zero = { 0 };
...
static PENDO_WEBP_INLINE void VerticalPred16x16(uint8_t* dst, const uint8_t* top) {
  if (top != NULL) {
...
static PENDO_WEBP_INLINE void HorizontalPred16x16(uint8_t* dst,
                                            const uint8_t* left) {
  if (left != NULL) {
    int j;
    for (j = 0; j < 16; j += 4) {
...
static PENDO_WEBP_INLINE void TrueMotion16x16(uint8_t* dst, const uint8_t* left,
                                        const uint8_t* top) {
  if (left != NULL) {
    if (top != NULL) {
      int j;
...
static PENDO_WEBP_INLINE void DCMode16x16(uint8_t* dst, const uint8_t* left,
                                    const uint8_t* top) {
  int DC;
  v16u8 out;
...
static PENDO_WEBP_INLINE void VerticalPred8x8(uint8_t* dst, const uint8_t* top) {
  if (top != NULL) {
...
static PENDO_WEBP_INLINE void HorizontalPred8x8(uint8_t* dst, const uint8_t* left) {
  if (left != NULL) {
    int j;
    for (j = 0; j < 8; j += 4) {
      const v16u8 L0 = (v16u8)__msa_fill_b(left[0]);
...
static PENDO_WEBP_INLINE void TrueMotion8x8(uint8_t* dst, const uint8_t* left,
                                      const uint8_t* top) {
  if (left != NULL) {
    if (top != NULL) {
      int j;
...
static PENDO_WEBP_INLINE void DCMode8x8(uint8_t* dst, const uint8_t* left,
                                  const uint8_t* top) {
  uint64_t out;
  v16u8 src = { 0 };
  if (top != NULL && left != NULL) {
